...
function container_get_lines(data) {
	...
	for (var i = 0; i < array_length(data.components) && !hit_character_limit;) {
		...
		if (string_width(text_to_draw) > data.renderWidth - last_width) {
			...
			if (string_length(text_to_draw) == 0) { //if a single word is breaking apart the string display
				if (last_width == 0) { //panic, this segment can't be broken up neatly along a space
					text_to_draw = carryover;
					carryover = "";
					while ((string_width(text_to_draw) > data.renderWidth - last_width) && string_length(text_to_draw) > 0) { //similar process, but no longer picky about splitting up words
						last_char = string_char_at(text_to_draw, string_length(text_to_draw));
						carryover = last_char + carryover;
						text_to_draw = string_delete(text_to_draw,string_length(text_to_draw),1);
					}
					if (string_length(text_to_draw) == 0 && last_width == 0) { //desperation tactic if a string is too small to display correctly
						text_to_draw = carryover;
						carryover = "";
					}
				}
		...
		if (!is_undefined(carryover)) { // includes blank carryover, for when a new component is also a new line, but a space separates them
